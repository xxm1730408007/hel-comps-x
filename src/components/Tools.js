 ...
 */
export function handleTree(data, id, parentId, children, rootId, isSet) {
  // 去除重复项
  if (isSet) {
 ...
 * 精简版是否开启
 */
export function getLiteVersionEnable() {
  if (process.env.VUE_APP_LITE_VERSION_ENABLE === "true") {
    return true;
  }
  if (process.env.VUE_APP_LITE_VERSION_ENABLE === "false") {
    return false;
  }
  return process.env.VUE_APP_LITE_VERSION_ENABLE || false;
}
